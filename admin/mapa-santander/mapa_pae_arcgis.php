...
// Obtener año de la vigencia (por defecto todos) - para uso futuro con API
$ano = isset($_GET['ano']) ? trim($_GET['ano']) : 'todos';

// Usar el método original que funciona (datos locales de tbl_pae)
...
<body>
	...
	<div id="contenido-mapa" class="cuerpoMapa w-12">
		...
		<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 788.66 885.68" width="100%" height="auto">

			<?php foreach ($santander as $key => $value) : ?>
			<?php 
                // Usar el nuevo campo homologado con la API de ArcGIS PAE
                $nombreApi = !empty($value['nombre_api_arcgis_pae']) 
                    ? $value['nombre_api_arcgis_pae']
                    : strtoupper(str_replace([' ', '-'], ['_'], removeAccentsPAE($value['municipio'])));
            ?>
			<g id="<?php echo strtoupper($value['path']); ?>">
				<path id="<?php echo strtoupper($value['path']); ?>" d="<?php echo $value['d']; ?>"
					fill="<?php echo getColorByNumPAEArcgis($value["total"]) ?>"
					class="carmen-del-chucuri municipios mapaClick pae-mapa-municipio"
					data-url="<?php echo getUrl() . 'estado_pae_municipios.php?mun=' . $value['codigo_muncipio'] . '&dep=' . $value['codigo_departamento']; ?>"
					data-municipio="<?php echo htmlspecialchars($value['nombre_api_arcgis_pae']); ?>"
