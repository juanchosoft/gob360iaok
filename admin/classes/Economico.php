...
class Economico {
    ...
    public static function getAll($rqst){
        ...
        $q = "SELECT * FROM " . $db->getTable('tbl_economico');
        if ($id > 0) {
            $q = "SELECT * FROM " . $db->getTable('tbl_economico') . " WHERE id = " . $id;
        }
        $result = $pdo->query($q);
        $arr = array();
        if ($result) {
    ...
    public static function save($rqst)
    {
        ...
        if ($id > 0) {
            ...
            $q = "SELECT id FROM " . $db->getTable('tbl_economico') . " WHERE id = " . $id;
            $result = $pdo->query($q);
            if ($result) {
                $table = $db->getTable('tbl_economico');
            ...
            if ($nombre != "") {
                $q = "INSERT INTO " . $db->getTable('tbl_economico') . " (created_at, nombre, tipo, puntaje, porcentaje,  tbl_usuario_id)
