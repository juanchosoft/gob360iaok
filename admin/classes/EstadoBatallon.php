 ...
 */
class EstadoBatallon
{

  /**
   * Metodo para listar los municipios de cada Batallon
   */
  public static function getMunicipiosXBatallon($rqst)
  {
    ...
    if ($id > 0) {
      ...
      $q = "SELECT 
          tbl_batallones.id, 
          tbl_batallones_x_municipio.tbl_batallones_id, 
          tbl_batallones_x_municipio.tbl_brigada_id, 
          tbl_batallones.sigla, 
          tbl_batallones_x_municipio.carpeta_svg, 
          tbl_batallones_x_municipio.nombre_svg, 
          tbl_ciudades.color, 
          tbl_ciudades.puntaje,
          tbl_ciudades.id as tbl_ciudad_id,
          tbl_ciudades.municipio,
          tbl_ciudades.codigo_muncipio,
          tbl_ciudades.codigo_departamento
          FROM (" . $db->getTable('tbl_batallones_x_municipio') . "  
          INNER JOIN " . $db->getTable('tbl_batallones') . " ON tbl_batallones_x_municipio.tbl_batallones_id = tbl_batallones.id) 
          INNER JOIN " . $db->getTable('tbl_ciudades') . " ON tbl_batallones_x_municipio.tbl_ciudad_id = tbl_ciudades.id
      ...
      if ($result) {
        ...
        // Cantidad de veredas por color  ACTUAL
        $q1 = "SELECT tbl_vereda.color, COUNT(tbl_vereda.color) AS cuenta, tbl_batallones.id AS tbl_batallon_id  
              FROM " . $db->getTable('tbl_vereda') . " INNER JOIN " . $db->getTable('tbl_batallones') . "  ON tbl_vereda.tbl_batallon_id = tbl_batallones.id
              WHERE tbl_batallones.id = $id AND NOT tbl_vereda.color ='0' GROUP BY tbl_vereda.color ORDER BY cuenta ASC";
        ...
        // Cantidad de veredas por color 2021
        $q2021 = "SELECT tbl_vereda.color2021, COUNT(tbl_vereda.color2021) AS cuenta, tbl_batallones.id AS tbl_batallon_id  
              FROM " . $db->getTable('tbl_vereda') . " INNER JOIN " . $db->getTable('tbl_batallones') . "  ON tbl_vereda.tbl_batallon_id = tbl_batallones.id
              WHERE tbl_batallones.id = $id AND NOT tbl_vereda.color2021 ='0' GROUP BY tbl_vereda.color2021 ORDER BY cuenta ASC";
        ...
        foreach ($arr2021 as $valor2021) {
          foreach ($arr1 as $valor2022) {
            ...
            if ($color2021 == "#387905" && $color2021 == $color2022) {
              $estable = round((($valor2022['cuenta'] - $valor2021['cuenta']) / $valor2022['cuenta']) * 100, 2);
              $estableEstado = Util::validarEstadoColorV2($valor2021['cuenta'], $valor2022['cuenta']);
            }
            // Bajo
            if ($color2021 == "#0041FE" && $color2021 == $color2022) {
              $bajo =  round((($valor2022['cuenta'] - $valor2021['cuenta']) / $valor2022['cuenta']) * 100, 2);
              $bajoEstado = Util::validarEstadoColorV2($valor2021['cuenta'], $valor2022['cuenta']);
            }
            // Medio
            if ($color2021 == "#FEE300" &&  $color2021 == $color2022) {
              $medio =  round((($valor2022['cuenta'] - $valor2021['cuenta']) / $valor2022['cuenta']) * 100, 2);
              $medioEstado = Util::validarEstadoColorV2($valor2021['cuenta'], $valor2022['cuenta']);
            }
            // Alto
            if ($color2021 == "#F2860D" &&  $color2021 == $color2022) {
              $alto = round((($valor2022['cuenta'] - $valor2021['cuenta']) / $valor2022['cuenta']) * 100, 2);
              $altoEstado = Util::validarEstadoColorV2($valor2021['cuenta'], $valor2022['cuenta']);
            }
            // Critico
            if ($color2021 == "#FC0707" && $color2021 == $color2022) {
              $critico =  round((($valor2022['cuenta'] - $valor2021['cuenta']) / $valor2022['cuenta']) * 100, 2);
        ...
        // Informacion de vereda y municipio 
        $q2 = "SELECT tbl_brigadas.id, 
              tbl_brigadas.sigla AS brigada, 
              tbl_batallones.sigla AS batallon, 
              tbl_ciudades.municipio, 
              Count(tbl_vereda.id) AS cantidad
        ...
        // Se consulta la información del estado DESPLAZAMIENTO Y OCUPACION
        $qEstado = "SELECT  tbl_batallones.id, tbl_brigadas.sigla AS brigada, tbl_votaciones.tiempo_ocupacion, tbl_votaciones.nombre_puesto, tbl_batallones.sigla AS batallon,
        ...
        if ($resultEstado) {
          ...
          if ($totalLlenosDesplazamiento > 0) {
        ...
        $arrjson = array('output' => array(
          ...
          'cantidadesPorDivision_2021' => $arr2021,
          'veredas_municipio_x_batallon' => $arr2,
          'porcentaje' => $promedio,
          'promedioDesplazamiento' => $promedioDesplazamiento,
          'estable' => $estable,
          'bajo' => $bajo,
          'medio' => $medio,
          'alto' => $alto,
   ...
   */
  public static function getPuntajeByBatallonId($rqst)
  {

    $tbl_batallon_id = isset($rqst['tbl_batallon_id']) ? intval($rqst['tbl_batallon_id']) : '';
    ...
    if ($tbl_batallon_id > 0) {
      ...
      $q0 = "SELECT id, puntaje, color FROM " . $db->getTable('tbl_veredas') . "  WHERE tbl_batallon_id = '$tbl_batallon_id'";
  ...
  /**
   * Obtener las veredas por color del año 2021 hacia atras
   */
  public static function getVeredasPorColorBatallon2021($rqst)
  {
    ...
    if ($tbl_batallon_id > 0  && $color != "") {

      $db = new DbConection();
      $pdo = $db->openConect();

      $q = "SELECT tbl_vereda.nombre_vereda, tbl_vereda.color2021, tbl_batallones.sigla AS batallon, tbl_ciudades.municipio, tbl_departamentos.departamento,
        tbl_brigadas.sigla AS brigada
        FROM " . $db->getTable('tbl_vereda') . " INNER JOIN " . $db->getTable('tbl_batallones') . "  ON tbl_vereda.tbl_batallon_id = tbl_batallones.id
        ...
        INNER JOIN  " . $db->getTable('tbl_brigadas') . "  ON tbl_vereda.tbl_brigada_id = tbl_brigadas.id
        WHERE tbl_batallones.id = $tbl_batallon_id AND NOT tbl_vereda.color2021 ='0' AND tbl_vereda.color2021 = '$color' ";
      $result = $pdo->query($q);
      $arr = array();
      if ($result) {
        foreach ($result as $valor) {
  ...
  public static function getVeredasPorColorBatallon($rqst)
  {
    ...
    if ($tbl_batallon_id > 0  && $color != "") {
      ...
      $q = "SELECT tbl_vereda.nombre_vereda, tbl_vereda.color, tbl_batallones.sigla AS batallon, tbl_ciudades.municipio, tbl_departamentos.departamento,
        tbl_brigadas.sigla AS brigada
        FROM " . $db->getTable('tbl_vereda') . " INNER JOIN " . $db->getTable('tbl_batallones') . "  ON tbl_vereda.tbl_batallon_id = tbl_batallones.id
        INNER JOIN " . $db->getTable('tbl_ciudades') . "  ON tbl_vereda.municipio_id = tbl_ciudades.codigo_muncipio
        INNER JOIN " . $db->getTable('tbl_departamentos') . "  ON tbl_ciudades.codigo_departamento = tbl_departamentos.codigo_departamento
        INNER JOIN  " . $db->getTable('tbl_brigadas') . "  ON tbl_vereda.tbl_brigada_id = tbl_brigadas.id
        WHERE tbl_batallones.id = $tbl_batallon_id AND NOT tbl_vereda.color ='0' AND tbl_vereda.color = '$color' ";
      $result = $pdo->query($q);
      $arr = array();
      if ($result) {
        foreach ($result as $valor) {
  ...
  /**
   * Metodo para obtener solo los municipios por Batallon Id
   */
  public static function getSoloMunicipiosByBatallonById($rqst)
  {
    $id = isset($rqst['id']) ? intval($rqst['id']) : '';
  ...
  /**
   * Metodo para obtener solo los veredas por Batallon Id
   */

  public static function getSoloVeredasByBatallonById($rqst)
  {
    $id = isset($rqst['id']) ? intval($rqst['id']) : '';
    ...
    if ($id > 0) {
      ...
      $q = "SELECT tbl_batallones.id AS batallon_id, tbl_batallones.sigla, tbl_vereda.id AS tbl_vereda_id, tbl_vereda.nombre_vereda
        FROM " . $db->getTable('tbl_batallones') . " INNER JOIN " . $db->getTable('tbl_vereda') . "  ON tbl_batallones.id = tbl_vereda.tbl_batallon_id
        WHERE tbl_batallones.id = $id";
      $result = $pdo->query($q);
      $arr = array();
      if ($result) {
