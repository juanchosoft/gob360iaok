...
class Candidatos
{
  ...
  public static function getAll($rqst)
  {
    ...
    $q = "SELECT * FROM " . $db->getTable('candidatos');
    if ($id > 0) {
      $q = "SELECT * FROM " . $db->getTable('candidatos') . " WHERE id = " . $id;
    }

    if ($tipo != "") {
      $q = "SELECT * FROM " . $db->getTable('candidatos') . " WHERE tipo = '$tipo' AND habilitado = 'si'";
    }
    $result = $pdo->query($q);
    $arr = array();
    if ($result) {
  ...
  public static function available($rqst)
  {
    ...
    $q = "SELECT * FROM " . $db->getTable('candidatos') . " WHERE identificacion_num  = :identificacion_num ";
  ...
  public static function save($rqst)
  {
    ...
    $aspirante = isset($rqst['aspirante']) ? ($rqst['aspirante']) : '';
    ...
    if ($id > 0) {
      $q0 = "SELECT * FROM " . $db->getTable('candidatos') . " WHERE id = " . $id;
      ...
      if ($nombre != "" || $apellido != "" || $tbl_departamento_id > 0 || $identificacion_num != "") {
        $q = "INSERT INTO " . $db->getTable('candidatos') . " (created_at, nombre, apellido, identificacion_tipo, identificacion_num, partido, aspirante, telefono, img, tbl_departamento_id, tbl_municipio_id,  habilitado, tbl_usuario_id) VALUES
                                              ( " . Util::date_now_server() . ", :nombre, :apellido, :identificacion_tipo, :identificacion_num, :partido, :aspirante, :telefono, :img, :tbl_departamento_id, :tbl_municipio_id,  :habilitado, :tbl_usuario_id)";
        $result = $pdo->prepare($q);
        $arrparam = array(
          ':nombre' => $nombre,
          ':apellido' => $apellido,
