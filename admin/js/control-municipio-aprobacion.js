...
function cargarCompromiso() {
  ...
  let columnsDefinition = [
    ...
    },
    {
      data: "id",
      render: function (data) {
        return `<form action="reporte-compromiso.php" method="POST" style="display:inline;">
...
function mostrarInformacionCompromiso(compromiso) {
  ...
  $("#estado_modal").val(compromiso.estado || "");

  let infoHTML = `
    <div class="card mb-3">
      <div class="card-header bg-primary text-white">
        <h6 class="mb-0"><i class="fas fa-info-circle"></i> Información del Compromiso</h6>
      </div>
      <div class="card-body">
        <div class="row">
          <div class="col-md-6">
            <p><strong>Secretaría:</strong> ${compromiso.secretaria || 'N/A'}</p>
            <p><strong>Municipio:</strong> ${compromiso.municipio || 'N/A'}</p>
            <p><strong>Componente:</strong> ${compromiso.componente || 'N/A'}</p>
            <p><strong>Estado a validar:</strong> <span class="badge badge-${getEstadoBadgeClass(estadoParaAutorizar)}">${estadoParaAutorizar || 'N/A'}</span></p>
          </div>
          <div class="col-md-6">
            <p><strong>Compromiso:</strong></p>
            <div class="border p-2 bg-light rounded" style="max-height: 100px; overflow-y: auto;">
              ${compromiso.compromisos || 'N/A'}
            </div>
            <p class="mt-2"><strong>Compromiso Pactado:</strong></p>
            <div class="border p-2 bg-light rounded" style="max-height: 100px; overflow-y: auto;">
...
function guardarObservacion() {
  ...
  let estadoModal = $("#estado_modal").val();
  if (!observacion) {
    Swal.fire({
      icon: 'warning',
  ...
  if (!aprobacion) {
    Swal.fire({
      icon: 'warning',
      title: 'Campo requerido',
      text: 'Por favor seleccione si desea aprobar o no.'
    });
    return;
  }

  if (!estadoModal || estadoModal === '' || estadoModal === 'Seleccione') {
    Swal.fire({
      icon: 'warning',
      title: 'Campo requerido',
      text: 'Por favor seleccione un estado válido para el compromiso.'
    });
    return;
  }
  ...
  const data = {
    'compromisoId': compromisoActualId,
    'secretariaId': secretariaIdObs,
    'municipio': municipio,
                ...
                <form id="formEditVisita">
                  <div class="form-row">
                    ...
                    <div class="form-group col-md-4">
                      ...
                      <input type="date" class="form-control" id="fecha" readonly value="${response.data[0].date ? response.data[0].date.split(' ')[0] : ""
        }">
                    </div>
