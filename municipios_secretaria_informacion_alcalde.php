...
if (isset($_REQUEST['mun'], $_REQUEST['dep'], $_REQUEST['secretaria']) && !empty(trim($_REQUEST['mun'])) && !empty(trim($_REQUEST['dep'])) && !empty(trim($_REQUEST['secretaria']))) {
    ...
    // Parámetro opcional de vereda para filtrar
    $vereda = isset($_REQUEST['vereda']) && !empty(trim($_REQUEST['vereda'])) ? trim($_REQUEST['vereda']) : null;

    // Información de proyectos de ALCALDÍA (con filtro opcional de vereda)
    $filtrosProyectos = array('municipioId' => $municipio, 'secretariaId' => $secretaria);
    if ($vereda) {
        $filtrosProyectos['veredaId'] = $vereda;
    }
    // Usar la función específica para proyectos de alcaldías
    $secretariasMunicipioProyectos = MunicipioSecretaria::getProyectosAlcaldiaPorSecretaria($filtrosProyectos);
    ...
    // Obtener nombre de la vereda para mostrar en el título
    $nombreVereda = null;
    if ($vereda && isset($secretariasMunicipioProyectos['output']['response']['vereda'])) {
        $nombreVereda = $secretariasMunicipioProyectos['output']['response']['vereda']['nombre_vereda'] ?? null;
    }
} else { ?>
                ...
                <div class="col-sm-12">
                    <div class="card">
                        ...
                        <div class="col-sm-12" id="divConsolidado">
                            ...
                            $secretariaInfo = $secretariasMunicipioProyectos['output']['response']['secretaria'] ?? null;
                            $veredaInfo = $secretariasMunicipioProyectos['output']['response']['vereda'] ?? null;
                            $proyectos = $secretariasMunicipioProyectos['output']['response']['proyectos'] ?? [];
                            $totalProyectos = $secretariasMunicipioProyectos['output']['response']['total_proyectos'] ?? 0;
                            ?>

                            <!-- Título con información de filtros -->
                            <h5 class="mb-3">
                                Proyectos de Alcaldía
                                <?php if ($secretariaInfo): ?>
                                    - <?= htmlspecialchars($secretariaInfo['secretaria']) ?>
                                <?php endif; ?>
                                <?php if ($veredaInfo): ?>
                                    <span class="badge badge-info ml-2">Vereda: <?= htmlspecialchars($veredaInfo['nombre_vereda']) ?></span>
                                <?php endif; ?>
                            </h5>
                            ...
                            <?php if (!empty($proyectos)): ?>
                                <div class="card">
                                    <div class="card-header">
                                        <span class="badge badge-primary"><?= $totalProyectos ?> proyecto(s) encontrado(s)</span>
                                    </div>
                                    <div class="card-body">
                                                        ...
                                                        <tr>
                                                            <td>
                                                                <button type="button" class="btn btn-sm btn-primary"
                                                                    title="Editar"
                                                                    onclick="location.href='detalle_proyectos_alcaldias.php?id=<?= htmlspecialchars($proyecto['tbl_proyecto_id'], ENT_QUOTES, 'UTF-8') ?>&nombre=<?= urlencode($proyecto['proyecto']) ?>'">
                                                                    <i class="feather icon-edit"></i>
                                                                </button>
                                                            </td>
                                                            <td><?= htmlspecialchars($proyecto['tbl_proyecto_id']) ?></td>
                                                            <td><?= htmlspecialchars($proyecto['proyecto']) ?></td>
                                                            <td><?= htmlspecialchars($proyecto['nombre_vereda'] ?? 'Sin vereda') ?></td>
                                                            <td><?= htmlspecialchars($proyecto['observaciones'] ?? '') ?></td>
                                                            <td>$<?= htmlspecialchars(number_format($proyecto['valor_proyecto'] ?? 0, 2)) ?></td>
                                                            <td><?= htmlspecialchars($proyecto['porcentaje_ejecucion'] ?? 0) ?>%</td>
                                                            <td><?= htmlspecialchars($proyecto['fecha_entrega'] ?? '') ?></td>
                                                            <td>
                                                                <?php
                                                                $estado = $proyecto['estado'] ?? '';
                                                                $badgeClass = 'badge-secondary';
                                                                if (stripos($estado, 'formulación') !== false) $badgeClass = 'badge-warning';
                                                                elseif (stripos($estado, 'ejecución') !== false) $badgeClass = 'badge-info';
                                                                elseif (stripos($estado, 'terminado') !== false) $badgeClass = 'badge-success';
                                                                ?>
                            ...
                            <?php else: ?>
                                <div class="alert alert-info">
                                    ...
                                    <?php if ($secretariaInfo): ?>
                                        para la secretaría <strong><?= htmlspecialchars($secretariaInfo['secretaria']) ?></strong>
                                    <?php endif; ?>
                                    <?php if ($veredaInfo): ?>
                                        en la vereda <strong><?= htmlspecialchars($veredaInfo['nombre_vereda']) ?></strong>
                                    <?php endif; ?>
                                </div>
                            <?php endif; ?>
