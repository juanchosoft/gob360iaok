...
$modulo = 'Asesor Contratación - Estructurador de Proyectos IA';
...
$mensajeInicial = $saludo . ' ' . $nombreUsuario . ', ¿en qué vamos a trabajar el día de hoy?';

/*
  WEBHOOKS N8N
  IMPORTANTE:
  1. En el workflow nuevo de n8n deja el Webhook principal con este path:
     contratacion-analizar-documento
  2. El workflow debe estar Published / Active.
  3. El botón de generar documento NO usa n8n; genera el Word directo desde el navegador.
*/
$webhookAnalizar = 'https://n8n.spidersoftwareia.com/webhook/contratacion-analizar-documento';
$webhookChat = 'https://n8n.spidersoftwareia.com/webhook/f26221b0-5ca3-4cc9-8a52-c9b2f608d516/chat';
?>
...
<body>
          ...
          <div class="au-hero">
            ...
            <div class="au-hero__content">
              ...
              <div class="au-subtitle">
                Asistencia especializada en contratación estatal, estudios previos, riesgos, presupuestos y estructuración de proyectos.
              </div>
            </div>
          </div>
        ...
        <div class="col-lg-4 col-md-12 mb-3">
          <div class="document-panel">
            <div class="document-panel__header">
              <h5>Analizar documento de contratación</h5>
              <p>Sube PDF para recibir concepto contractual y apoyo en estructuración de proyectos.</p>
            </div>
            ...
            <form id="formDocumento" enctype="multipart/form-data" method="post">
              <div class="form-group">
                ...
                <textarea
                  ...
                  class="form-control"
                  rows="3"
                  placeholder="Ejemplo: Revisa estos estudios previos y dime riesgos, modalidad de selección, ajustes contractuales y recomendaciones."
                ></textarea>
              </div>
            ...
            <div id="resultadoDocumento" class="document-result" style="display:none;">
              <h5 id="tituloResultadoDocumento">Concepto contractual</h5>
              <div id="conceptoDocumento"></div>

              <div class="document-actions" id="accionesDocumento" style="display:none;">
                <p><strong>¿Deseas generar este análisis en documento?</strong></p>
  ...
  <script>
    ...
    function obtenerTextoConcepto(data) {
      ...
      const campos = [
        ['Resumen ejecutivo', data.resumen_ejecutivo || data.resumen],
        ['Problema o necesidad', data.problema_necesidad || data.problema_juridico],
        ['Análisis jurídico contractual', data.analisis_contractual || data.analisis_juridico],
        ['Modalidad de selección', data.modalidad_seleccion],
        ['Objeto y alcance', data.objeto_alcance || data.objeto],
        ['Presupuesto y valor estimado', data.presupuesto || data.valor_estimado],
        ['Matriz de riesgos', data.matriz_riesgos || data.riesgos],
        ['Obligaciones sugeridas', data.obligaciones],
        ['Recomendaciones', data.recomendaciones],
        ['Ruta de acción', data.ruta_accion],
        ['Concepto final', data.concepto_final || data.concepto || data.respuesta || data.output || data.text || data.message]
      ];
      ...
      campos.forEach(([titulo, valor]) => {
        ...
        if (Array.isArray(valor)) {
          texto = valor.map(v => String(v ?? '').trim()).filter(Boolean).join('\n- ');
          if (texto) texto = '- ' + texto;
        } else if (typeof valor === 'object') {
          texto = JSON.stringify(valor, null, 2);
        } else {
      ...
      return data.error || data.mensaje || JSON.stringify(data, null, 2);
    }
    ...
    function mostrarResultado(texto, esError = false) {
      ...
      tituloResultadoDocumento.textContent = esError ? 'Error del análisis' : 'Concepto contractual';
    ...
    function descargarWordDesdeConcepto(data) {
      ...
      const resumen = textoPlanoDocumento(data.resumen_ejecutivo || data.resumen);
      const problema = textoPlanoDocumento(data.problema_necesidad || data.problema_juridico);
      const analisis = textoPlanoDocumento(data.analisis_contractual || data.analisis_juridico);
      const modalidad = textoPlanoDocumento(data.modalidad_seleccion);
      const objeto = textoPlanoDocumento(data.objeto_alcance || data.objeto);
      const presupuesto = textoPlanoDocumento(data.presupuesto || data.valor_estimado);
      const riesgos = data.matriz_riesgos || data.riesgos || [];
      const obligaciones = data.obligaciones || [];
      const recomendaciones = data.recomendaciones || [];
      const pruebas = data.documentos_soporte || data.pruebas_necesarias || [];
...
<head>
  <meta charset="UTF-8">
  <title>Análisis Contractual y Estructuración de Proyecto</title>
  <style>
    @page {
      margin: 2.5cm;
    }
...
<body>
  ...
  <h1>Análisis Contractual y Estructuración de Proyecto</h1>

  <div class="asunto">
    <strong>Asunto:</strong> Concepto técnico-jurídico en contratación estatal y estructuración de proyectos.
  </div>

  <h2>1. Resumen ejecutivo</h2>
  <p>${parrafoDocumento(resumen, 'No se suministró un resumen ejecutivo específico.')}</p>

  <h2>2. Problema, necesidad o conveniencia</h2>
  <p>${parrafoDocumento(problema, 'No se identificó una necesidad específica con la información suministrada.')}</p>

  <h2>3. Análisis jurídico contractual</h2>
  <p>${parrafoDocumento(analisis || concepto, 'No se generó análisis contractual específico.')}</p>

  <h2>4. Modalidad de selección sugerida</h2>
  <p>${parrafoDocumento(modalidad, 'No se definió una modalidad de selección específica.')}</p>

  <h2>5. Objeto, alcance y entregables</h2>
  <p>${parrafoDocumento(objeto, 'No se definió objeto o alcance específico.')}</p>

  <h2>6. Presupuesto y aspectos financieros</h2>
  <p>${parrafoDocumento(presupuesto, 'No se identificó información presupuestal específica.')}</p>

  <h2>7. Matriz de riesgos</h2>
  ${listaDocumento(riesgos)}

  <h2>8. Obligaciones sugeridas</h2>
  ${listaDocumento(obligaciones)}

  <h2>9. Documentos o soportes necesarios</h2>
  ${listaDocumento(pruebas)}

  <h2>10. Recomendaciones</h2>
  ${listaDocumento(recomendaciones)}

  <h2>11. Ruta de acción sugerida</h2>
  <p>${parrafoDocumento(ruta, 'No se definió una ruta de acción específica.')}</p>

  <h2>12. Concepto final</h2>
  ...
  <div class="firma">
    ...
    <br><br>
    <p>
      <strong>Asesor IA en Contratación Estatal</strong><br>
      Especialista en Derecho Público, Derecho Administrativo,<br>
      Contratación Estatal y Estructuración de Proyectos.
    </p>
  </div>

  <p class="nota">
    Este documento corresponde a una orientación técnico-jurídica generada con apoyo de IA.
    No reemplaza la revisión integral, personalizada y definitiva de un abogado o estructurador con acceso completo
    a los documentos, estudios previos, estudios de mercado, presupuesto, matriz de riesgos y antecedentes del proceso.
  </p>
...
`;
      ...
      const nombreArchivo = 'analisis-contractual-proyecto-' + Date.now() + '.doc';
    ...
    formDocumento.addEventListener('submit', async function (event) {
      ...
      formData.append('consulta', consulta || 'Analiza este documento como experto en contratación estatal y estructuración de proyectos.');
      ...
      formData.append('tipo_asesor', 'contratacion_estructuracion_proyectos');
    ...
    btnSoloRespuesta.addEventListener('click', function () {
      estadoDocumento.textContent = 'Listo. El análisis quedó como respuesta en pantalla.';
    });
    ...
    if (chatTarget) {
      ...
      try {
        createChat({
          ...
          i18n: {
            es: {
              title: 'Contratación IA',
              subtitle: 'Asesor contractual y estructurador de proyectos',
              footer: '',
              getStarted: 'Iniciar conversación',
              inputPlaceholder: 'Escribe tu consulta de contratación o estructuración...'
            }
          }
        });
