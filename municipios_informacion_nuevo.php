...
if (isset($_REQUEST['mun'], $_REQUEST['dep'], $_REQUEST['secretaria_nuevo'], $_REQUEST['accion_nuevo']) && !empty(trim($_REQUEST['mun'])) && !empty(trim($_REQUEST['dep']))) {

    $municipio = trim($_REQUEST['mun']);
    $departamento = trim($_REQUEST['dep']);
    $secretaria_nuevo = trim($_REQUEST['secretaria_nuevo']);
    $accion_nuevo = trim($_REQUEST['accion_nuevo']);


    $dataActualizacion = ActualizacionInformacion::obtenerRegistrosActualizadosPorMunicipio(
        $departamento,
        $municipio,
        $secretaria_nuevo,
        $accion_nuevo
    );
    $registrosActualizados = $dataActualizacion['output']['response'] ?? [];
    $isValidActualizacion = $dataActualizacion['output']['valid'] ?? false;


    // Info secretarias y proyectos del municipio
    $secretariasMunicipioProyectos = MunicipioSecretaria::getProyectosPorSecretariaByMunicipioId(array('municipioId' => $municipio, 'secretariaId' => $secretaria_nuevo));


    $informacionPorSecretaria = SecretariasMunicipio::getAllSecretariaByCodigoMunicipio(array('secretariaId' => $secretaria_nuevo, 'codigoMunicipio' => $municipio));
                ...
                <div class="col-sm-12">
                    <div class="card">
                        ...
                        <div class="col-sm-12" id="divConsolidado">
                            ...
                            <div>
                                <h5 class="mb-3">Registro de Actualizaciones (Mapa Nuevo)</h5>
                                <div class="table-responsive">
                                    <table class="table">
                                        <thead>
                                            <tr>
                                        ...
                                        <tbody>
                                            <?php if ($isValidActualizacion && !empty($registrosActualizados)): ?>
                                                <?php foreach ($registrosActualizados as $item): ?>
                                                    <tr>
                                                        <td><?= htmlspecialchars($item['nombre_vereda'] ?? 'N/A') ?></td>
                                                        <td><?= htmlspecialchars($item['valor_actualizacion'] ?? 0) ?></td>
                                                        <td><?= htmlspecialchars($item['observaciones_actualizacion'] ?? 'Sin descripción') ?></td>
                                                        <td><?= htmlspecialchars($item['dtcreate'] ?? 'N/A') ?></td>
                                                    </tr>
                                                <?php endforeach; ?>
                                            <?php else: ?>
                                                <tr>
                                                    <td colspan="4" class="text-center text-muted">No se han registrado actualizaciones para estos parámetros.</td>
                                                </tr>
                                            <?php endif; ?>
                                        </tbody>
                                    </table>
